...
Command: npx gltfjsx@6.5.3 public/models/Chan-simple.glb --output src/components/ChanSimple.tsx --transform --types 
Files: public/models/Chan-simple.glb [1.41MB] > /Users/kalina/Projects/kalina/portfolio/src/components/Chan-simple-transformed.glb [251.12KB] (82%)
*/
...
type ChanSimpleProps = Readonly<React.JSX.IntrinsicElements["group"]> & {
  animationName?: string;
  timePosition?: number;
  loop?: boolean;
};

export function ChanSimple({
  ...
  const { scene: modelScene } = useGLTF("/models/Chan-simple-transformed.glb");
  const { animations } = useGLTF("animations/running_to_turn_mirror.glb");
  ...
  const mixer = useRef<THREE.AnimationMixer | null>(null); // on first render the mixer is null so we wait
  ...
  React.useEffect(() => {
    ...
    mixer.current ??= new THREE.AnimationMixer(nodes.mixamorigHips);
    ...
    return () => {
      action.stop(); // stop this animation when dependencies change or component unmounts
    };
...
useGLTF.preload("/models/Chan-simple-transformed.glb");
