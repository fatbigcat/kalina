...
function AnimatedLetter({ char, index, position, onMove, resetSignal }: AnimatedLetterProps) {
  ...
  return (
    <motion.span
      ref={letterRef}
      className="magnet-letter relative inline-block cursor-grab select-none active:cursor-grabbing"
      animate={controls}
      onTap={handleTap}
    ...
    >
      <span className="magnet-letter-rim" aria-hidden="true">{char}</span>
      <span className="magnet-letter-face">{char}</span>
    </motion.span>
  );
}
    ...
    <>
      ...
      <div className="fixed bottom-0 left-0 right-0 z-20 m-4 md:right-auto">
        <h1
          ...
          className="display_font m-0 w-full leading-none md:hidden"
          style={{
            width: '100%',
        ...
        <h1
          className="display_font m-0 hidden leading-none md:block"
          style={{
