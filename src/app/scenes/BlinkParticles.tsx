...
const BlinkMaterial = shaderMaterial(
  ...
  void main() {
    ...
    // Sample simplex noise in two directions
    float noiseX = snoise(vec3(position.x * 1.5, 0.0, uTime * 0.5 + phase));
    float noiseY = snoise(vec3(0.0, position.y * 1.5, uTime * 0.5 + phase));
    // Combine the two noise values (average)
    float combinedNoise = (noiseX + noiseY) * 0.5;
    float size = aScale + combinedNoise * 0.5;
...
function StaticParticles() {
  const gridSize = 25;
  const separation = 7;
  const count = gridSize * gridSize * 2 * gridSize;
    ...
    for (let x = 0; x < gridSize; x++) {
      for (let y = 0; y < gridSize; y++) {
        for (let z = 0; z < gridSize; z++) {
          ...
          scales[i] = (Math.sin(x) + 1) * 0.05; // Scale based on index for variety
          i++;
        }
      }
    }
